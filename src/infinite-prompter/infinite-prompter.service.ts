...
import { NodeExpressionToString } from 'src/athena-parser/expression-constructor.class';
...
export class InfinitePrompterService {
  ...
  async initiate() {
    while (true) {
      ...
      const nodeExpressionToString = new NodeExpressionToString(
        abstractSyntaxTree
      );
      console.log(nodeExpressionToString.getEvaluatedExpression());
    }
  }
}
